...
def getmajoritylocations(rows):
    ...
    loccounts = dict((i,list(locs).count(i)) for i in locs)
    maxnum = getmaxcount(loccounts)
    for key, val in loccounts.items():
        if val == maxnum:
            return key
...
def analyze(vectors, threshold):
    ...
    tuple = vectors[['my_event_terms', 'day_of_week', 'time_of_day', 'location', 'time_stamp','lat','long']]
    types = tuple.groupby('my_event_terms')
    numtuples = 0
    ...
    for i in dictionary:
        ...
        starttime = min(rows.iloc[:,5])
        endtime = max(rows.iloc[:,5])
        ...
        if impact >= threshold:
            ...
            print(line)
            writetofile(line, "dataset/my_extractedeventsfromtweets.csv")
            numtuples = numtuples + 1
    return numtuples

# Read all the tuples
# entities<>event_terms<>day_of_week<>time_of_day<>location<>tweet_text<>time_stamp<>lat<>long
#FILENAME = "dataset/final-training-data-nov.txt"
FILENAME = "dataset/annotated_events.csv"
df = pd.read_csv(FILENAME, delimiter="\t")
thresholdlist = [10]

# Generate the time stamps for the entire range of time the tweets were generated
print(df.iloc[1,5])
print(df.iloc[np.shape(df)[0] - 1, 5])

#start = pd.to_datetime('2013-07-30 19:03:27')
#end = pd.to_datetime('2013-08-01 13:00:09')
start = pd.to_datetime(df.iloc[1,5])
end = pd.to_datetime(df.iloc[np.shape(df)[0] - 1, 5])
